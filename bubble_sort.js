...
function formatNumbers() { // adds the numbers from unsortedArr to an array with spaces

  for (var i = 0; i < unsortedArr.length; i++){
    numbers.push(" " + unsortedArr[i]);
  }

}

formatNumbers(); // function is called

// ~ ~ start

var numbersnode = document.createTextNode(numbers); /* turns the numbers arrary into a text node 
                                                       so appendChild can work with it */

start.appendChild(numbersnode); /* appends the 'numbersnode' var, containing 
                                the text node of the nicely-formatted "numbers" var,
                                to the 'start' var, a <p> element */

document.getElementById("start").appendChild(start); /* appends the 'start' var, a <p> element,
                                                      to the <div> with the id='start' */



// = = = = = = = = = = ACTUAL BUBBLE SORT ALGORITHIM: = = = = = = = = = = = = 
...
function bubbleSort(unsortedArr) {
  ...
  for (var j = 0; j < unsortedArr.length; j++) {
    ...
    for (var k = 0; k < (unsortedArr.length - j - 1); k++) {

      if (unsortedArr[k] > [k + 1])
        {
          var n = unsortedArr[k];
...
bubbleSort(unsortedArr); // function call

function formatSortedNumbers() { // adds the numbers from unsortedArr to an array with spaces

  for (var i = 0; i < unsortedArr.length; i++){
    sortedNumbers.push(" " + unsortedArr[i]);
  }

}

formatSortedNumbers(); // function call


// ~ ~ printing result to the document

function printResult() {

var sortednode = document.createTextNode(sortedNumbers); 

result.appendChild(sortednode);
